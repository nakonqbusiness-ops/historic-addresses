(function(){
    ...
    // Auth is enforced on the SERVER now: we exchange the password for a signed
    // token via /api/login, then send it on every mutating request.
    const SESSION_TOKEN = 'sys_admin_token';
    const SESSION_ROLE  = 'sys_auth_role';
    ...
    async function checkAuth() {
        ...
        if (!input) { errorEl.textContent = 'Please enter password'; return; }
        ...
        try {
            ...
            if (res.ok) {
                ...
                showAdminPanel(data.role, data.name);
                return;
            }
...
function showAdminPanel(role, name) {
    ...
    window.addEventListener('DOMContentLoaded', function() {
        if (adminToken) {
            // Validate the stored token with the server before showing the panel
            _fetch('/api/me', { headers: { 'X-Admin-Token': adminToken } })
                .then(function(r) { return r.ok ? r.json() : Promise.reject(); })
                .then(function(me) { showAdminPanel(me.role, me.name); })
                .catch(function() {
                    sessionStorage.removeItem(SESSION_TOKEN);
                    sessionStorage.removeItem(SESSION_ROLE);
                    adminToken = '';
